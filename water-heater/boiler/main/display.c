...
void display_chart(uint8_t *points, uint8_t *points2, int num_points, int current, int hours,
                   float temp, float temp2, int thres, int mode)
{
    ...
    char *text;
    asprintf(&text, "[%02d:%02d] %.1f, %.1f~C [%d]%d", hours/100, hours%100, temp, temp2, mode, thres);
    lv_label_set_text(label, text);
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 10);
    ...
    for (int i = current; i < num_points; i++) {
        ...
        lv_chart_set_next_value(chart, ser3, 0x7F & points2[i]);
